...
% Setup some useful variables
    ...
    Index = eye(num_labels);
    yNormalised = zeros(m, num_labels);

    for i=1:m
      yNormalised(i, :)= Index(y(i), :);
    end


    J = (sum( sum( -1*yNormalised.*log(h) - (1 - yNormalised).*log(1-h) ) ))/m;

    Theta1New = Theta1(:,2:size(Theta1,2));
    Theta2New = Theta2(:,2:size(Theta2,2));

    R1 =sum(sum(Theta1New.^2));
    R2 = sum(sum(Theta2New.^2)); 
    Rtotal = (lambda/(2*m))*( R1 + R2 );
